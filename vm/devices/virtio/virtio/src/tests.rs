...
impl VirtioTestGuest {
    ...
    fn add_queue_memory(&self, queue_index: u16) {
        let packed_descriptors = matches!(self.info, VirtioQueueInfo::Packed(_));
        // descriptors
        for i in 0..self.queue_size {
            ...
            // length
            self.test_mem
                .modify_memory_map(base + 8, &0x1000u32.to_le_bytes(), packed_descriptors);
            // split: flags, packed: buffer_id
            self.test_mem
                .modify_memory_map(base + 12, &0u16.to_le_bytes(), packed_descriptors);
            // split: next index, packed: flags
            self.test_mem
                .modify_memory_map(base + 14, &0u16.to_le_bytes(), packed_descriptors);
        }
...
async fn verify_split_queue_indirect_linked(driver: DefaultDriver) {
    let test_mem = VirtioTestMemoryAccess::new();
    verify_queue_indirect_linked_inner(VirtioTestGuest::new_split(&driver, &test_mem, 1, 2, true))
        .await;
}
#[async_test]
async fn verify_packed_queue_indirect_linked(driver: DefaultDriver) {
    let test_mem = VirtioTestMemoryAccess::new();
    verify_queue_indirect_linked_inner(VirtioTestGuest::new_packed(&driver, &test_mem, 1, 2, true))
        .await;
}
...
async fn verify_device_queue_simple_inner(
    ...
    let mut dev = VirtioMmioDevice::new(
        Box::new(LegacyWrapper::new(
            &VmTaskDriverSource::new(SingleDriverBackend::new(guest.driver())),
            TestDevice::new(
                DeviceTraits {
                    device_id: 3,
...
async fn verify_device_multi_queue_pci_inner(
    ...
    let features = (features.bank(1) as u64) << 32 | features.bank(0) as u64;
    let base_addr: Vec<_> = (0..num_queues)
        .map(|i| guest.get_queue_descriptor_backing_memory_address(i))
        .collect();
