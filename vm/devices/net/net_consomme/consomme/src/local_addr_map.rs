...
//! consomme subnet.
//!
//! When a UDP or TCP reply arrives from a "local" address (loopback, the
//! guest's assigned IP, etc.), the old behavior collapsed all of them to the
//! single gateway IP. This loses information needed for the reverse path —
//! when the guest sends a packet *to* that translated address, we couldn't
//! determine the original real address.
//!
//! This module maintains a bidirectional mapping between real host addresses
//! and unique virtual addresses allocated from the subnet, enabling correct
//! reverse translation.
